        ...
        private ProblemDetails GetProblemDetails(Exception ex)
        {
            ...
            else
            {
                return new ProblemDetails
                {
                    ...
                    Detail = @"We apologize for inconvenience. Please let us know about the error at support@WebAPIExcercise.com. Include traceId: {traceId} in email",
                    Instance = traceId
                };
            }
        }

        private static InvalidRequestProblemDetails GetBadRequestProblemDetails(InvalidRequestException ex)
        {
            string traceId = Guid.NewGuid().ToString();
